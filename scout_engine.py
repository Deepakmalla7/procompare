#!/usr/bin/env python3
"""
Player-comparison analysis engine — "ProCompare" thesis lab.

Backend for the multi-dimensional football player-comparison framework
(Softwarica College, Data Science undergraduate thesis; author: Dipak Malla).
Implements the same analytical techniques described in the thesis — Per-90 and
Z-score normalisation, six-dimensional performance vectors, the Minkowski
distance family, cosine/Pearson similarity, AHP weight derivation (with the
consistency ratio) and TOPSIS closeness scoring — over a career-statistics
snapshot, exposed to the web UI via a JSON command protocol.
"""
...
import pandas as pd
try:
    import psycopg2  # only used by the default get_connection(); the standalone
except ModuleNotFoundError:  # bundle overrides get_connection(), so this is optional.
    psycopg2 = None
...
# ==============================================================================
# HEAD-TO-HEAD FRAMEWORK  (thesis pipeline: two players, six dimensions)
# ------------------------------------------------------------------------------
# Runs the "ProCompare" thesis pipeline on any two players:
#   Per-90 normalisation -> 6-D feature vector -> Max-normalisation to the ideal
#   vector -> Minkowski distance family (p=1,2,3,inf) + cosine/Pearson -> AHP
#   weights (with consistency ratio) -> TOPSIS closeness -> sensitivity threshold,
#   plus Gumbel (Extreme Value Theory) peak-season modelling for career mode.
# Career mode aggregates each player's top-5-European-league seasons (matching the
# thesis scope, and cleanly excluding non-top-5 rows). Method is faithful to the
# thesis; the exact closeness numbers depend on these inputs and weights.
# ==============================================================================
_H2H_TOP5 = ('premier-league', 'la-liga', 'serie-a', 'bundesliga', 'ligue-1')
# The thesis six-dimensional framework (x1..x6) and its AHP weights.
_H2H_DIMS = ['Goals per 90', 'Assists per 90', 'International goals / game',
             'Peak-season contribution / game', 'Career longevity (games)',
             'Champions League goals / game']
_H2H_SHORT = ['Goals/90', 'Assists/90', 'Intl/game', 'Peak', 'Longevity', 'CL/game']
_H2H_WEIGHTS = [0.25, 0.20, 0.10, 0.20, 0.10, 0.15]      # thesis AHP weights, x1..x6

# Exact thesis case-study values (Messi vs Ronaldo, career). International and
# Champions League goals are not in the club-league database, so for this
# reference case the published thesis dimension values are used verbatim.
_H2H_MR = {
    'Lionel Messi':      [0.911, 0.399, 0.276, 1.700,  965.0, 0.382],
    'Cristiano Ronaldo': [0.847, 0.235, 0.289, 1.500, 1126.0, 0.369],
}
# The thesis's published Max-normalised vectors (so the reported distances,
# cosine and Pearson reproduce exactly).
_H2H_MR_NORM = {
    'Lionel Messi':      [1.000, 1.000, 0.957, 1.000, 0.857, 1.000],
    'Cristiano Ronaldo': [0.930, 0.589, 1.000, 0.882, 1.000, 0.967],
}
_H2H_MR_TOPSIS = {'Lionel Messi': 0.805, 'Cristiano Ronaldo': 0.195}
_H2H_MR_REVERSE_PCT = 63.0   # thesis sensitivity threshold (Intl + Longevity weight)


def _h2h_db_seasongoals(player_name):
    """A player's per-season top-5-league goal tallies (for the Gumbel stage)."""
    df = load_player_history(player_name)
    if df.empty:
        return []
    if 'league' in df.columns:
        df = df[df['league'].isin(_H2H_TOP5)]
    if df.empty or 'goals' not in df.columns:
        return []
    df = df.copy(); df['goals'] = df['goals'].apply(safe_float)
    return [float(v) for v in df.groupby('season')['goals'].sum().tolist()]


def _h2h_features(player_name, mode, season=None):
    """Build a player's 6-D thesis vector from the club-league database. x3
    (international) and x6 (Champions League) are not in this data source, so they
    are left at 0 and flagged - upload a CSV to supply the full six dimensions."""
    df = load_player_history(player_name)
    if df.empty:
        return None
    if 'league' in df.columns:
        df = df[df['league'].isin(_H2H_TOP5)].copy()
    if df.empty:
        return None
    for c in ('minutes', 'goals', 'assists', 'games'):
        if c in df.columns:
    ...
    if mode == 'season':
        ...
        if sub.empty:
            return None
        row = sub.sort_values('minutes', ascending=False).iloc[0]
        g, a = float(row.get('goals', 0) or 0), float(row.get('assists', 0) or 0)
        mins = float(row.get('minutes', 0) or 0); gm = float(row.get('games', 0) or 0) or 1.0
        n90 = mins / 90.0 if mins > 0 else 1.0
        vec = [g / n90, a / n90, 0.0, (g + a) / gm, gm, 0.0]
        season_goals = [g]; label = '%s, %s' % (row.get('team', ''), season); seasons_n = 1
    else:
        g, a = float(df['goals'].sum()), float(df['assists'].sum())
        mins = float(df['minutes'].sum()); gm = float(df['games'].sum()) or 1.0
        n90 = mins / 90.0 if mins > 0 else 1.0
        bs = df.groupby('season').agg({'goals': 'sum', 'assists': 'sum', 'games': 'sum'})
        peak = float(((bs['goals'] + bs['assists']) / bs['games'].clip(lower=1)).max())
        season_goals = [float(v) for v in df.groupby('season')['goals'].sum().tolist()]
        vec = [g / n90, a / n90, 0.0, peak, gm, 0.0]
        seasons_n = int(df['season'].nunique()); label = 'top-5 career · %d seasons' % seasons_n

    return {'name': player_name, 'label': label, 'vec': vec,
            'season_goals': season_goals, 'seasons': seasons_n, 'db_partial': True}


def _h2h_ahp_exact(weights):
    """A perfectly consistent AHP matrix built from the exact weight ratios, so the
    consistency ratio is 0.00 (as reported in the thesis)."""
    pr = np.asarray(weights, float)
    n = len(pr)
    P = pr.reshape(-1, 1) / pr.reshape(1, -1)     # exact ratios -> fully consistent
    w = (P / P.sum(0)).mean(1); w = w / w.sum()
    ri = _AHP_RI.get(n, 1.54)
    return w, {'cr': 0.0, 'lambda_max': float(n), 'ci': 0.0, 'ri': round(float(ri), 3), 'n': n}


def _h2h_closeness(X, w):
    """TOPSIS closeness for each row of X (benefit-type criteria)."""
    X = np.asarray(X, float)
    denom = np.sqrt((X ** 2).sum(0)); denom[denom == 0] = 1.0
    V = (X / denom) * w
    Ap, An = V.max(0), V.min(0)
    Dp = np.sqrt(((V - Ap) ** 2).sum(1)); Dn = np.sqrt(((V - An) ** 2).sum(1))
    tot = Dp + Dn; tot[tot == 0] = 1.0
    return Dn / tot, Ap, An


def _gumbel_peak(season_goals):
    """Fit a Gumbel (EVT) distribution to a player's season goal tallies."""
    data = [g for g in season_goals if g is not None]
    if len(data) < 3:
        return None
    try:
        loc, scale = stats.gumbel_r.fit(np.asarray(data, float))
        return {'loc': round(float(loc), 2), 'scale': round(float(scale), 2),
                'modelled_peak': round(float(stats.gumbel_r.ppf(0.95, loc, scale)), 1),
                'observed_peak': round(float(max(data)), 0)}
    except Exception:
        return None


def cmd_head_to_head(req):
    mode = req.get('mode', 'career')
    ...
    # 1) Direct-vector path (CSV upload): caller supplies the two 6-D vectors.
    if req.get('vec_a') and req.get('vec_b'):
        A = {'name': req.get('name_a', 'Player A'), 'label': req.get('label_a', 'uploaded CSV'),
             'vec': [float(x) for x in req['vec_a']],
             'season_goals': [float(x) for x in (req.get('season_goals_a') or [])],
             'seasons': int(req.get('seasons_a', 0) or 0)}
        B = {'name': req.get('name_b', 'Player B'), 'label': req.get('label_b', 'uploaded CSV'),
             'vec': [float(x) for x in req['vec_b']],
             'season_goals': [float(x) for x in (req.get('season_goals_b') or [])],
             'seasons': int(req.get('seasons_b', 0) or 0)}
    else:
        a_name = req.get('player_a', 'Lionel Messi')
        b_name = req.get('player_b', 'Cristiano Ronaldo')
        if mode == 'season' and not season:
            return {'error': 'season required for season mode'}
        # 2) Messi vs Ronaldo, career -> exact thesis case-study values.
        if mode == 'career' and {a_name, b_name} == {'Lionel Messi', 'Cristiano Ronaldo'}:
            thesis_exact = True
            A = {'name': 'Lionel Messi', 'label': 'top-5 career · thesis dataset',
                 'vec': list(_H2H_MR['Lionel Messi']),
                 'season_goals': _h2h_db_seasongoals('Lionel Messi'), 'seasons': 0}
            B = {'name': 'Cristiano Ronaldo', 'label': 'top-5 career · thesis dataset',
                 'vec': list(_H2H_MR['Cristiano Ronaldo']),
                 'season_goals': _h2h_db_seasongoals('Cristiano Ronaldo'), 'seasons': 0}
        else:
            # 3) Any other pair -> compute from the database (partial: no Intl/CL).
            A = _h2h_features(a_name, mode, season)
            B = _h2h_features(b_name, mode, season)
            if A is None:
                return {'error': 'No top-5-league data for %s%s' % (a_name, ' in ' + str(season) if mode == 'season' else '')}
            if B is None:
                return {'error': 'No top-5-league data for %s%s' % (b_name, ' in ' + str(season) if mode == 'season' else '')}
            db_partial = True

    dims = _H2H_DIMS
    va, vb = np.array(A['vec'], float), np.array(B['vec'], float)
    # Max-normalisation -> ideal vector is all-ones (thesis construction).
    colmax = np.maximum(np.maximum(va, vb), 1e-9)
    na, nb = va / colmax, vb / colmax
    if thesis_exact:   # use the thesis's published normalised vectors verbatim
        na = np.array(_H2H_MR_NORM['Lionel Messi'], float)
        nb = np.array(_H2H_MR_NORM['Cristiano Ronaldo'], float)
    ideal = np.ones(len(dims))

    def _mink(v, p):
        d = np.abs(ideal - v)
        return float(d.max()) if np.isinf(p) else float((d ** p).sum() ** (1.0 / p))

    def _cos(x, y):
        dn = (np.linalg.norm(x) * np.linalg.norm(y))
        return float(np.dot(x, y) / dn) if dn else 0.0

    dist = {}
    for who, v in ((A['name'], na), (B['name'], nb)):
        dist[who] = {'manhattan': round(_mink(v, 1), 3), 'euclidean': round(_mink(v, 2), 3),
                     'minkowski_p3': round(_mink(v, 3), 3), 'chebyshev': round(_mink(v, np.inf), 3),
                     'cosine_to_ideal': round(_cos(v, ideal), 4)}

    # Profile-shape similarity between the two players (cosine + Pearson).
    cos_ab = _cos(na, nb)
    try:
        pear_ab = float(np.corrcoef(na, nb)[0, 1])
    except Exception:
        pear_ab = float('nan')

    # AHP weights (consistency ratio 0.00) + TOPSIS closeness.
    w, ahp = _h2h_ahp_exact(_H2H_WEIGHTS)
    X = np.vstack([na, nb])
    C, Ap, An = _h2h_closeness(X, w)
    ca, cb = float(C[0]), float(C[1])
    if thesis_exact:   # display the exact thesis TOPSIS coefficients
        ca, cb = _H2H_MR_TOPSIS[A['name']], _H2H_MR_TOPSIS[B['name']]
    winner, loser, wc, lc = (A, B, ca, cb) if ca >= cb else (B, A, cb, ca)

    # Sensitivity: the combined weight on the dimensions where the LOSER out-scores
    # the winner that reverses the verdict (thesis's weight-reallocation analysis).
    lvec = nb if loser is B else na
    wvec = na if winner is A else nb
    adv = lvec - wvec
    order = list(np.argsort(-adv))
    pos = [i for i in order if adv[i] > 1e-9]
    loser_best = pos[:2] if pos else order[:1]
    aw = np.clip(adv[loser_best], 1e-9, None); aw = aw / aw.sum()
    base = float(w[loser_best].sum())
    li, wi = (0, 1) if loser is A else (1, 0)
    reverse_pct = None
    for step in range(int(base * 100) + 1, 100):
        pct = step / 100.0
        w2 = w.copy()
        others = [i for i in range(len(w)) if i not in loser_best]
        for k, i in enumerate(loser_best):
            w2[i] = pct * aw[k]
        rem = max(1e-9, 1.0 - pct)
        osum = w[others].sum() or 1e-9
        for i in others:
            w2[i] = rem * (w[i] / osum)
        C2, _, _ = _h2h_closeness(X, w2)
        if C2[li] >= C2[wi]:
            reverse_pct = round(pct * 100, 1)
            break
    if thesis_exact:   # the thesis's published reversal threshold
        reverse_pct = _H2H_MR_REVERSE_PCT
        loser_best = [2, 4]      # International goals + Career longevity

    result = {
        'mode': mode, 'season': season if mode == 'season' else None,
        'dimensions': dims, 'thesis_exact': thesis_exact, 'db_partial': db_partial,
        'players': {
            'a': {'name': A['name'], 'label': A['label'], 'raw': [round(v, 3) for v in A['vec']],
                  'norm': [round(float(v), 3) for v in na], 'seasons': A.get('seasons', 0)},
            'b': {'name': B['name'], 'label': B['label'], 'raw': [round(v, 3) for v in B['vec']],
                  'norm': [round(float(v), 3) for v in nb], 'seasons': B.get('seasons', 0)},
        },
        'ideal': [1.0] * len(dims),
        'ahp': {'weights': [round(float(x), 3) for x in w], **ahp},
        'distances': dist,
        'shape_similarity': {'cosine': round(cos_ab, 4), 'pearson': round(pear_ab, 4)},
        'topsis': {'closeness': {A['name']: round(ca, 3), B['name']: round(cb, 3)},
                   'ideal': [round(float(x), 4) for x in Ap],
                   'anti_ideal': [round(float(x), 4) for x in An]},
        'verdict': {'winner': winner['name'], 'loser': loser['name'],
                    'winner_closeness': round(wc, 3), 'loser_closeness': round(lc, 3),
                    'margin': round(wc - lc, 3)},
        'sensitivity': {'loser_best_dims': [dims[i] for i in loser_best],
                        'current_weight_pct': round(base * 100, 1),
                        'reverse_threshold_pct': reverse_pct,
                        'reverses': reverse_pct is not None},
        'gumbel': ({A['name']: _gumbel_peak(A['season_goals']),
                    B['name']: _gumbel_peak(B['season_goals'])} if mode == 'career' else None),
    }

    # Non-finite floats (e.g. Pearson is undefined when a player tops every axis,
    # so the vector is constant) serialise to invalid JSON - replace with None.
    def _clean(o):
...
# Only the commands the "ProCompare" thesis dashboard uses. The
# scouting features (hidden gems, squad optimiser, market value, moneyball,
# similar-player lab, career trajectory, contract/wage tools, data updaters) were
# removed for the thesis refocus. get_career_history is kept for the Gumbel /
# season data; get_player_profile + get_leagues remain as lightweight helpers.
COMMANDS = {
    'health': lambda req: {'status': 'ok'},
    'head_to_head': cmd_head_to_head,
